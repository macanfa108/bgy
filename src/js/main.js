    ...
    $(function() {
      /*--------首页--------*/
        ...
        $("#navbar2 .fruitBasket").off().on("click",function(){
          //加入果篮操作，获取对应产品的规格数量，加入购物车后调用layer插件显示结果
        ...
        if(ulDom.length){
          ...
          ulDom.on('click','.item',function(e){
            ...
            if(target=='a'){
              ...
              alert('选择当前位置后可跳转回首页位置！进行相应城市位置的设置');
            }
          });
        }
        ...
        /*--------地址管理页面--------*/
        $("#addressManagementCon .iconfont-group").on("click",".icon-tianjia",function(){
          //对应的父级元素
        ...
        /*--------选择城市页面--------*/
        //三级联动功能
        $(".selectionCon").each(function(){
            var url = "js/area.json";//模拟城市，后期根据接口进行改动或重写功能
            var areaJson;
            var temp_html;
            ...
            var district = function(){
                ...
                if(typeof(areaJson[m].city[n].d) == "undefined"){
                    oDistrict.css("display","none");
                }else{
                    oDistrict.css("display","inline");
                    $.each(areaJson[m].city[n].d,function(i,district){
        ...
        //修改昵称
        $("#nickname").off("click").on("click",function(){
          var html=$("#changeNickname").html();//自定义模板
          layer.open({
            anim:"up",
            type:0,
            // closeBtn: false,
            ...
            btn:['确认','取消'],
            yes: function(index){
              //之前获取不到新的Input值：加载自定义模板的时候重新复制了一份到弹窗中，获取到的不是新生成的Input控件
              var nicknameInput=$(".layui-m-layersection .nicknameInput").val();//输入框的值
              // console.log(nicknameInput);
              //$.ajax();
              layer.open({
                content: '先执行ajax异步修改，之后弹窗提示结果!input值：'+nicknameInput
                ,time:3
              });
              layer.close(index);
            }
        ...
        $("#Sex").on("click",function(){
          var changeSexHtml=$("#changeSex").html();//自定义模板
          layer.open({
            anim:"up",
            type:0,
            shadeClose:true,
        ...
        /*--------绑定手机页面--------*/
        if($("#bindingPhone").length){
          validator.init({id:"bindingPhone",btnId:"bindingPhoneBtn",callback:function(){
            console.log("绑定手机!");
            // 模拟跳转
        ...
        /*--------更换手机页面--------*/
        if($("#replacePhone").length){
          validator.init({id:"replacePhone",btnId:"replacePhoneBtn",callback:function(){
            alert("更换手机操作!");
            // 模拟跳转
            // window.location.href="https://www.baidu.com";
            /*
        ...
        $(".unwrapBtn").on("click",function(){
          layer.open({
            ...
            ,btn:['确定','取消']
            ,yes:function(index){
              // $.ajax({});在请求成功函数中进行下列弹窗的关闭和结果弹窗的是提示
              layer.close(index);
              layer.open({
                content:"解绑成功!"
                ,time:2
        ...
        /*--------商品评价webuploader上传图片--------*/
        //评分
        var starDoms=$(".gradeCon .star");
        starDoms.on("click",function(){
          var _index=$(this).index();

          for (var i = 0; i < starDoms.length; i++) {
            if(i<=_index){
              $(starDoms[i]).addClass("active");
            }else{
              $(starDoms[i]).removeClass("active");
            }
          }
          //隐藏input控件赋值
          $("#startScore").val(_index+1);
